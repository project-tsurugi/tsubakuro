...
/**
 * FutureContinueImpl type.
 */
public class FutureContinueImpl implements FutureResponse<Void> {
    FutureInputStream futureInputStream;

    public FutureContinueImpl(FutureInputStream futureInputStream) {
    ...
    public Void get() throws IOException, ServerException, InterruptedException {
        try {
            var response = DatastoreResponseProtos.BackupContinue.parseDelimitedFrom(futureInputStream.get());

            if (DatastoreResponseProtos.BackupContinue.ResultCase.SUCCESS.equals(response.getResultCase())) {
                return null;
            }
