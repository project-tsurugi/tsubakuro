...
public class Delivery {
	...
	while (!stop.get()) {
	    ...
	    for (dId = 1; dId <= Scale.DISTRICTS; dId++) {
                var ps1 = RequestProtos.ParameterSet.newBuilder()  // "SELECT no_o_id FROM NEW_ORDER WHERE no_d_id = :no_d_id AND no_w_id = :no_w_id ORDER BY no_o_id"
                ...
                var ps3 = RequestProtos.ParameterSet.newBuilder()  // "SELECT o_c_id FROM ORDERS WHERE o_id = :o_id AND o_d_id = :o_d_id AND o_w_id = :o_w_id"
                ...
                var ps5 = RequestProtos.ParameterSet.newBuilder()  // "UPDATE ORDER_LINE SET ol_delivery_d = :ol_delivery_d WHERE ol_o_id = :ol_o_id AND ol_d_id = :ol_d_id AND ol_w_id = :ol_w_id"
                ...
                var ps6 = RequestProtos.ParameterSet.newBuilder()  // "SELECT o_c_id FROM ORDERS WHERE o_id = :o_id AND o_d_id = :o_d_id AND o_w_id = :o_w_id"
                    .addParameters(RequestProtos.ParameterSet.Parameter.newBuilder().setName("ol_o_id").setInt8Value(noOid))
                    .addParameters(RequestProtos.ParameterSet.Parameter.newBuilder().setName("ol_d_id").setInt8Value(dId))
                    .addParameters(RequestProtos.ParameterSet.Parameter.newBuilder().setName("ol_w_id").setInt8Value(paramsWid));
