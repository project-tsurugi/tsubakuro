...
* `interface BackupDetail`
  ...
  * `getLogStart()`
    * overview
      * 利用可能なログの開始時刻を返す
    * returns
      * `long` - ログの開始時刻 (符号なし。内部時刻のため、日付等にはマッピング不可)
    * note
      * この値は主に増分を計算する際に、ログに欠落がないかを判断するためのものである
  * `getLogFinish()`
    * overview
      * 利用可能なログの終了時刻を返す
    * returns
      * `long` - ログの終了時刻 (符号なし。内部時刻のため、日付等にはマッピング不可)
    * note
      * クラッシュしてログが壊れている場合、この情報を利用して末尾のトリムを行うかもしれない
  * `getImageFinish()`
    * overview
      * 利用可能なデータベースイメージの終了時刻を返す
    * returns
      * `long` - データベースイメージの終了時刻 (符号なし。内部時刻のため、日付等にはマッピング不可)
      * empty - データベースイメージがバックアップ対象に含まれない場合
    * note
      * この値は主に増分を計算する際に、ログに欠落がないかを判断するためのものである
      * `v1.getImageFinish() < v2.getLogStart() + 1` が成り立つ場合、 v1 から v2 への増分はログに欠落が存在する
  * `keepAlive()`
    * overview
